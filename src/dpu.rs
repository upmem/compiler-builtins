...
intrinsics! {
    pub extern "C" fn __mulsi3(a: u32, b: u32) -> u32 {
        let c: u32;
        unsafe {
            llvm_asm!("  jgtu $2, $1, __mulsi3_swap
                    move r2, $1
...
pub unsafe fn __udiv32() {
    llvm_asm!("  clz r3, r1, max, __udiv32_division_by_zero
            clz r4, r0
...
macro_rules! atomic_cmpxchg {
    ($name:ident, $ty:ty) => {
        ...
        pub unsafe extern "C" fn $name(ptr: *mut $ty, oldval: $ty, newval: $ty) -> $ty {
            llvm_asm!("acquire zero, $0, nz, ." :: "i" (&LLVM_ATOMIC_LOCK) :: "volatile");
            let curval = *ptr;
            if curval == oldval {
                *ptr = newval;
            }
            llvm_asm!("release zero, $0, nz, . + 1" :: "i" (&LLVM_ATOMIC_LOCK) :: "volatile");
            return curval;
        }
    };
}
